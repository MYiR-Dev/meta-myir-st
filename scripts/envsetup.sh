...
eula_check() {
    ...
    local machine_file=$(find ${ROOTOE}/$_META_LAYER_ROOT/ -type d \( -name '.git' -o -name '.repo' -o -name 'build*' -o -name 'source*' -o -name 'script*' \) -prune -o -type f -name "$MACHINE.conf" | grep "/machine/$MACHINE.conf")
...
get_templateconf()
{
	...
	local machine_path=$(find ${ROOTOE}/layers/meta-myir-st/ -type d \( -name '.git' -o -name '.repo' -o -name 'build*' -o -name 'source*' -o -name 'script*' \) -prune -o -type f -name "$MACHINE.conf" | grep "/machine/$MACHINE.conf" | sed 's|\(.*\)/conf/machine/\(.*\)|\1|')
	...
	if [ "$(echo $machine_path | wc -w)" -gt 1 ]; then
            echo ""
	...
	if [ -f $machine_path/conf/template/bblayers.conf.sample ]; then
		_TEMPLATECONF=$machine_path/conf/template
        else
